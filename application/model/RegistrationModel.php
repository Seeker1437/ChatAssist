...
class RegistrationModel
{
	...
	public static function sendVerificationEmail($user_id, $user_email, $user_activation_hash)
	{
		...
		$body = str_replace('{{email_verification_link}}',
            Config::get('URL') . Config::get('EMAIL_VERIFICATION_URL') . '/' . urlencode($user_id) . '/' . urlencode($user_activation_hash),
            file_get_contents(Config::get('PATH_VIEW') . '_templates/email/accountVerificationEmail.php')
        );

		$mail = new Mail;

		// Get an email object to make customer changes to it
		$message = $mail->getPhpMailerObject(Filter::XSSFilter($user_email), Config::get('EMAIL_VERIFICATION_FROM_EMAIL'), "ALFC Reporting", "ALFC Live Stream Report", $body);
		$message->isHTML();

		$mail_sent = $mail->sendPhpMailerObject($message);
